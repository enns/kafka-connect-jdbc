...
public class TableQuerierTest {
  ...
  public void testTimestampIncrementingTableQuerierInTableModeWithSuffix() throws SQLException {
    ...
    String query = "SELECT * FROM \"name\" WHERE \"name\".\"column\" > ? ORDER BY \"name\".\"column\" ASC /* SUFFIX */";
    when(databaseDialectMock.optimizeSelectQuery(query)).thenReturn(query);
    querier.createPreparedStatement(connectionMock);

    verify(databaseDialectMock, times(1)).createPreparedStatement(Matchers.any(),Matchers.eq(query));
  }
